...
Usage example:
----------------------------------------------------------------
# Example usage (remove this circular import):
# from tokenization.arc_tokenizer import get_or_build_arc_tokenizer, demo_arc_tokenizer_test
...
def get_or_build_arc_tokenizer(save_dir: str = "arc_tokenizer_v1") -> PreTrainedTokenizerFast:
    ...
    def decode_to_grid(self, token_string):
        token_string = re.sub(r'<s>|</s>|<pad>', '', token_string).strip()
        rows = token_string.split('<arc_nl>')
        grid = []
        for row in rows:
            tokens = row.split('><')
            row_values = []
            for token in tokens:
                ...
                if token.startswith('arc_'):
                    try:
                        row_values.append(int(token.split('_')[1]))
                    except (ValueError, IndexError):
                        pass # Ignore non-numeric arc tokens
            if row_values:
                grid.append(row_values)
        return np.array(grid)

    # Add the method to the class
    PreTrainedTokenizerFast.decode_to_grid = decode_to_grid
